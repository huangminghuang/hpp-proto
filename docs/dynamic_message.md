...
*   **Message Field Presence**: When reading scalar, string, bytes, or enum fields, accessing an unset field will return its default value. However, for nested message fields, `has_value()`/`operator bool()` and `value()`/`get<message_value_cref>()` are checked, but `operator*`/`operator->` are unchecked — like `std::optional`, dereferencing an absent message field is undefined behavior. Always check presence, with `value()`/`get()` if you are not certain the field is set.
*   **Oneof Aliasing**: All members of a `oneof` share one storage slot. Unlike `std::optional`, that slot can be mutated through a *different* field reference into the same oneof — selecting a sibling invalidates every reference you previously obtained to the formerly-active member, even ones you already checked with `has_value()`. Re-check presence (or use `value()`/`get()`) immediately before each dereference; never cache a `has_value()` result, or an `operator*`/`operator->` result, across code that might touch a sibling of the same oneof.
