...
`protoc-gen-hpp` normally consumes a `CodeGeneratorRequest` produced by `protoc`; its optional request-file input is intended for trusted exported requests. The generator relies on `protoc` for descriptor structure and semantic validation. Descriptor strings are still treated as data and encoded for their generated C++ context rather than assumed to be valid C++ source fragments.
...
- <a id="plugin-options">PLUGIN_OPTIONS</a>: A comma-separated string forwarded to the protoc-gen-hpp plugin to customize code generation. Options include:
  ...
  - `proto2_explicit_presence=`: For Proto2 only, makes optional scalar fields implicitly present except for specified scopes. This option can be specified multiple times. For example: the option `proto2_explicit_presence=.pkg1.msg1.field1,proto2_explicit_presence=.pkg1.msg2` instructs the code generator that explicit presence is only applicable for the `field1` of `pkg1.msg1` and all fields of `pkg1.msg2`. If this option flattens an optional closed enum into a scalar, its generated binary metadata treats numeric values as open so the parser can retain values outside the declared enum. Applications that require the schema's closed-enum constraint must explicitly call the generated `is_valid(value)` function.
...
## Generated C++ names are public API

Every generated namespace and declaration name is considered public API. The generator therefore keeps names readable
and never substitutes hexadecimal or otherwise opaque encoded identifiers.

For protobuf package components, messages, fields, enums, enum values, oneofs, services, and RPC methods, exact C++
keywords follow the Google Protobuf C++ mapping and receive one trailing underscore: `class` becomes `class_`, for
example. Other protobuf identifiers retain their spelling. Names containing `__` or beginning with `_` followed by an
uppercase letter are rejected because C++ reserves those patterns in every scope. A lowercase-leading-underscore name,
such as `_field`, remains readable when emitted as a class member.

The keyword mapping is intentionally readable rather than injective: `class` and `class_` both map to `class_`. The
generator also creates documented helper names such as the `Outer_` namespace for the nested declarations of `Outer`.
If two protobuf declarations map to the same C++ name, or a protobuf declaration collides with a generated helper,
generation fails with a diagnostic asking the schema author to rename one declaration. It never hides the collision
behind a private-looking or encoded namespace.

## File descriptors

Generated `.desc.hpp` headers expose each file descriptor under a namespace derived from the virtual `.proto` path. This namespace is independent of the protobuf `package` and always begins with `hpp_proto::file_descriptors`. For example, `foo/bar.proto` declares:

```cpp
hpp_proto::file_descriptors::foo::bar_proto::file_descriptor_
hpp_proto::file_descriptors::foo::bar_proto::file_descriptor_set()
```

Directory components become nested namespaces, and the complete filename becomes the final namespace component. Bytes that cannot occur in a C++ identifier are replaced with `_`. Exact C++ keywords use the same mapping as the Google Protobuf C++ generator and receive a trailing `_`; for example, `class/value.proto` uses `class_::value_proto`. If normalization produces an implementation-reserved C++ identifier, generation fails with guidance to rename the path or use an override.

This normalization is not injective. For example, `file-name.proto` and `file_name.proto` both map to `file_name_proto`, while `class/value.proto` and `class_/value.proto` both use the `class_` directory namespace. When files visible in one generator request would declare the same C++ entity, or an entity would conflict with another descriptor namespace, generation fails with a diagnostic identifying both files. Rename a file or directory to resolve the collision, or override the final filename namespace component with the file option:

```proto
import "hpp_proto/hpp_options.proto";

option (hpp_proto.hpp_file_opts).file_descriptor_name = "special_bar_proto";
```

For `foo/bar.proto`, this example produces `hpp_proto::file_descriptors::foo::special_bar_proto`. The option must be a single identifier spelling. Exact C++ keywords receive the trailing-underscore mapping described above, and the resulting identifier must not contain `__` or begin with `_` followed by an uppercase letter. The option changes only the final namespace component; the fixed `hpp_proto::file_descriptors` prefix and the path's directory namespaces remain unchanged.

Collision checking covers all files present in the current `CodeGeneratorRequest`, including its dependency closure. Unrelated files generated by separate `protoc` invocations are not visible to one another, so builds that combine separately generated descriptor headers must keep their virtual paths and overrides unique.

Only the path-namespaced `file_descriptor_` and `file_descriptor_set()` API is generated. Regenerate descriptor headers for an entire dependency closure when adopting this API, because generated descriptor sets refer to dependencies by their path-namespaced names.
...
Notice how the top-level `Outer` template aliases the nested types via `using Inner = ::Outer_::Inner<Traits>;` and `using Kind = ::Outer_::Kind;`. The actual implementations live in the public, underscore-suffixed `Outer_` namespace without relying on an anonymous namespace, while the aliases preserve the original Proto scoping semantics. A sibling declaration that would also map to `Outer_` is rejected as a C++ name collision.
