...
const ut::suite varint_decode_tests = [] {
  ...
  "unchecked_parse_varint"_test =
      [](int64_t arg) {
        ...
        const auto *r = hpp::proto::shift_mix_parse_varint<int64_t>(
            data.data(),
            data.data() + data.size(), // NOLINT(cppcoreguidelines-pro-bounds-pointer-arithmetic)
            parsed_value);
        ut::expect(r == end);
      } |
      std::vector<int64_t>{
