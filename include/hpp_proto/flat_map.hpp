...
struct qualifies_as_range : std::false_type {};
template <class T>
struct qualifies_as_range<T, decltype(std::begin(std::declval<T()>()()), void(), std::end(std::declval<T()>()()),
                                      void())> : std::true_type {};

template <class It>
using is_random_access_iterator =
    std::is_convertible<typename std::iterator_traits<It>::iterator_category, std::random_access_iterator_tag>;
...
template <class T, class Alloc, class... Args,
          class = typename std::enable_if<
              ...
              std::is_constructible<T, std::allocator_arg_t, const Alloc &, Args &&...>::value>::type>
...
template <class T, class Alloc, class... Args,
          class = typename std::enable_if<std::uses_allocator<T, Alloc>::value &&
                                          std::is_constructible<T, Args &&..., const Alloc &>::value>::type>
...
using iter_key_type =
    typename std::remove_const<typename std::iterator_traits<InputIterator>::value_type::first_type>::type;
...
template <class A>
struct qualifies_as_allocator<A, void_t<typename A::value_type, decltype(std::declval<A &>().allocate(size_t{}))>>
    : std::true_type {};
...
public:
  ...
  using value_type = std::pair<const typename std::iterator_traits<KeyIt>::value_type,
                               typename std::iterator_traits<MappedIt>::value_type>;
  using reference =
      std::pair<typename std::iterator_traits<KeyIt>::reference, typename std::iterator_traits<MappedIt>::reference>;
  ...
  template <class CK, class CM, class = typename std::enable_if<std::is_convertible<CK, KeyIt>::value>::type,
  ...
  reference operator*() const { return reference{*kit_, *vit_}; }

  pointer operator->() const { return arrow_proxy<reference>{reference{*kit_, *vit_}}; }
  ...
  friend iter operator+(iter it, ptrdiff_t n) {
    it += n;
    return it;
  }
  friend iter operator+(ptrdiff_t n, iter it) {
    it += n;
    return it;
  }
  friend iter operator-(iter it, ptrdiff_t n) {
...
private:
  ...
  explicit iter(KeyIt &&kit, MappedIt &&vit) : kit_(static_cast<KeyIt &&>(kit)), vit_(static_cast<MappedIt &&>(vit)) {}
  KeyIt kit_;
  MappedIt vit_;
};
...
template <class Key, class Mapped, class Compare = std::less<Key>, class KeyContainer = std::vector<Key>,
...
class flat_map {
  ...
  static_assert(std::is_convertible<decltype(std::declval<const Compare &>()(std::declval<const Key &>(),
...
public:
  ...
  flat_map(KeyContainer keys, MappedContainer values, const Alloc &a)
      : flat_map(KeyContainer(static_cast<KeyContainer &&>(keys), a),
                 MappedContainer(static_cast<MappedContainer &&>(values), a)) {}

  template <class Container,
            typename std::enable_if<flatmap_detail::qualifies_as_range<const Container &>::value, int>::type = 0>
  explicit flat_map(const Container &cont) : flat_map(std::begin(cont), std::end(cont), Compare()) {}

  template <class Container,
            typename std::enable_if<flatmap_detail::qualifies_as_range<const Container &>::value, int>::type = 0>
  explicit flat_map(const Container &cont, const Compare &comp) : flat_map(std::begin(cont), std::end(cont), comp) {}
  ...
  flat_map(const Container &cont, const Alloc &a) : flat_map(std::begin(cont), std::end(cont), Compare(), a) {}
  ...
  flat_map(sorted_unique_t s, KeyContainer keys, MappedContainer values, const Alloc &a)
      : flat_map(s, KeyContainer(static_cast<KeyContainer &&>(keys), a),
                 MappedContainer(static_cast<MappedContainer &&>(values), a)) {}

  template <class Container,
            class = typename std::enable_if<flatmap_detail::qualifies_as_range<const Container &>::value>::type>
  flat_map(sorted_unique_t s, const Container &cont) : flat_map(s, std::begin(cont), std::end(cont), Compare()) {}
  ...
  explicit flat_map(const Compare &comp) : c_{}, compare_(comp) {}

  template <class Alloc, typename std::enable_if<std::uses_allocator<KeyContainer, Alloc>::value &&
                                                     std::uses_allocator<MappedContainer, Alloc>::value,
                                                 int>::type = 0>
  flat_map(const Compare &comp, const Alloc &a)
      : c_{flatmap_detail::make_obj_using_allocator<KeyContainer>(a),
  ...
  template <class InputIterator, class Alloc,
            ...
            class = typename std::enable_if<std::uses_allocator<KeyContainer, Alloc>::value>::type,
            class = typename std::enable_if<std::uses_allocator<MappedContainer, Alloc>::value>::type>
  flat_map(InputIterator first, InputIterator last, const Alloc &a) : flat_map(first, last, Compare(), a) {}
  ...
  flat_map(flat_map &&m, const Alloc &a)
      : c_{KeyContainer(static_cast<KeyContainer &&>(m.c_.keys), a),
  ...
  flat_map(std::initializer_list<value_type> &&il, const Compare &comp = Compare()) : flat_map(il, comp) {}

  template <class Alloc, typename std::enable_if<std::uses_allocator<KeyContainer, Alloc>::value &&
                                                     std::uses_allocator<MappedContainer, Alloc>::value,
                                                 int>::type = 0>
  flat_map(std::initializer_list<value_type> &&il, const Compare &comp, const Alloc &a) : flat_map(il, comp, a) {}

  template <class Alloc, typename std::enable_if<std::uses_allocator<KeyContainer, Alloc>::value &&
                                                     std::uses_allocator<MappedContainer, Alloc>::value,
                                                 int>::type = 0>
  flat_map(std::initializer_list<value_type> &&il, const Alloc &a) : flat_map(il, Compare(), a) {}
  ...
  template <class Alloc, typename std::enable_if<std::uses_allocator<KeyContainer, Alloc>::value &&
                                                     std::uses_allocator<MappedContainer, Alloc>::value,
                                                 int>::type = 0>
  flat_map(sorted_unique_t s, std::initializer_list<value_type> &&il, const Alloc &a) : flat_map(s, il, Compare(), a) {}
  ...
  mapped_reference operator[](const Key &x) { return try_emplace(x).first->second; }

  mapped_reference operator[](Key &&x) { return try_emplace(static_cast<Key &&>(x)).first->second; }
  ...
  std::pair<iterator, bool> insert(const value_type &x) { return this->emplace(x); }

  std::pair<iterator, bool> insert(value_type &&x) { return this->emplace(static_cast<value_type &&>(x)); }

  iterator insert(const_iterator position, const value_type &x) { return this->emplace_hint(position, x); }

  iterator insert(const_iterator position, value_type &&x) {
    return this->emplace_hint(position, static_cast<value_type &&>(x));
  }

  template <class P, class = decltype(std::pair<Key, Mapped>(std::declval<P &&>()))>
  ...
  void insert(std::initializer_list<value_type> il) { this->insert(il.begin(), il.end()); }
  ...
  const KeyContainer &keys() const { return c_.keys; }

  const MappedContainer &values() const { return c_.values; }
  ...
  template <class K, class Compare_ = Compare, class = typename Compare_::is_transparent>
  ...
  size_type count(const Key &k) const { return this->contains(k) ? 1 : 0; }
  ...
  iterator lower_bound(const Key &k) {
    auto kit =
        std::partition_point(c_.keys.begin(), c_.keys.end(), [&](const auto &elt) { return bool(compare_(elt, k)); });
    auto vit = c_.values.begin() + (kit - c_.keys.begin());
  ...
  iterator lower_bound(const K &x) {
    auto kit =
        std::partition_point(c_.keys.begin(), c_.keys.end(), [&](const auto &elt) { return bool(compare_(elt, x)); });
    auto vit = c_.values.begin() + (kit - c_.keys.begin());
    return flatmap_detail::make_iterator(kit, vit);
  }
  ...
  iterator upper_bound(const Key &k) {
    auto kit =
        std::partition_point(c_.keys.begin(), c_.keys.end(), [&](const auto &elt) { return !bool(compare_(k, elt)); });
    auto vit = c_.values.begin() + (kit - c_.keys.begin());
    return flatmap_detail::make_iterator(kit, vit);
  }
  ...
  iterator upper_bound(const K &x) {
    auto kit =
        std::partition_point(c_.keys.begin(), c_.keys.end(), [&](const auto &elt) { return !bool(compare_(x, elt)); });
    auto vit = c_.values.begin() + (kit - c_.keys.begin());
    return flatmap_detail::make_iterator(kit, vit);
  }
  ...
  std::pair<iterator, iterator> equal_range(const Key &k) {
    auto kit1 =
        std::partition_point(c_.keys.begin(), c_.keys.end(), [&](const auto &elt) { return bool(compare_(elt, k)); });
    auto kit2 = std::partition_point(kit1, c_.keys.end(), [&](const auto &elt) { return !bool(compare_(k, elt)); });
    auto vit1 = c_.values.begin() + (kit1 - c_.keys.begin());
    auto vit2 = c_.values.begin() + (kit2 - c_.keys.begin());
    return {flatmap_detail::make_iterator(kit1, vit1), flatmap_detail::make_iterator(kit2, vit2)};
  }
  ...
  std::pair<iterator, iterator> equal_range(const K &x) {
    auto kit1 =
        std::partition_point(c_.keys.begin(), c_.keys.end(), [&](const auto &elt) { return bool(compare_(elt, x)); });
    auto kit2 = std::partition_point(kit1, c_.keys.end(), [&](const auto &elt) { return !bool(compare_(x, elt)); });
    auto vit1 = c_.values.begin() + (kit1 - c_.keys.begin());
    auto vit2 = c_.values.begin() + (kit2 - c_.keys.begin());
    return {flatmap_detail::make_iterator(kit1, vit1), flatmap_detail::make_iterator(kit2, vit2)};
  }
...
template <class Key, class Mapped, class Compare, class KeyContainer, class MappedContainer>
void swap(flat_map<Key, Mapped, Compare, KeyContainer, MappedContainer> &x,
          flat_map<Key, Mapped, Compare, KeyContainer, MappedContainer> &y) noexcept(noexcept(x.swap(y))) {
  return x.swap(y);
}
...
template <class InputIterator, class Compare = std::less<flatmap_detail::iter_key_type<InputIterator>>,
          class = std::enable_if_t<flatmap_detail::qualifies_as_input_iterator<InputIterator>::value &&
                                   !flatmap_detail::qualifies_as_allocator<Compare>::value>>
...
flat_map(std::initializer_list<std::pair<const Key, T>>, Compare = Compare()) -> flat_map<Key, T, Compare>;

template <class Key, class T, class Compare, class Allocator,
          class = std::enable_if_t<!flatmap_detail::qualifies_as_allocator<Compare>::value &&
                                   flatmap_detail::qualifies_as_allocator<Allocator>::value>>
flat_map(std::initializer_list<std::pair<const Key, T>>, Compare, Allocator) -> flat_map<Key, T, Compare>;

template <class Key, class T, class Allocator,
          class = std::enable_if_t<flatmap_detail::qualifies_as_allocator<Allocator>::value>>
flat_map(std::initializer_list<std::pair<const Key, T>>, Allocator, int = 0 /*to please MSVC*/) -> flat_map<Key, T>;
...
template <class Key, class Mapped, class Compare = std::less<Key>, class KeyContainer = std::vector<Key>,
          class MappedContainer = std::vector<Mapped>>
bool empty_range(const flat_map<Key, Mapped, Compare, KeyContainer, MappedContainer> &r) {
  return r.empty();
}
