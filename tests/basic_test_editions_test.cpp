...
void test_basic_test_editions() {
  ...
  static_assert(!hpp_proto::concepts::optional<decltype(message.implicit)>);
  static_assert(hpp_proto::concepts::optional<decltype(message.explicit_)>);
...
void test_editions_test() {
  ...
  using meta_type = decltype(pb_meta(message));
  static_assert(hpp_proto::concepts::optional<decltype(message.plain_field)>);
  static_assert(!hpp_proto::concepts::optional<decltype(message.implicit_presence_field)>);
  static_assert(!hpp_proto::concepts::optional<decltype(message.required_field)>);
  static_assert(hpp_proto::concepts::optional<decltype(message.delimited_field)>);
  static_assert(std::tuple_element_t<3, meta_type>::is_delimited());

  static_assert(hpp_proto::concepts::optional<decltype(message.closed_enum_field)>);
  static_assert(std::tuple_element_t<4, meta_type>::closed_enum());

  static_assert(!hpp_proto::concepts::optional<decltype(message.open_enum_field)>);
