...
using TestMap = protobuf_unittest::TestMap<>;

inline void SetMapFields(TestMap *message) {
  // Add first element.
  ...
  message->map_bool_bool[false] = false;
  ...
  message->map_bool_bool[true] = true;
...
inline void ExpectMapFieldsSet(const TestMap &message) {
