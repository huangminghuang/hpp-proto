...
inline void SetAll(protobuf_unittest::TestAllExtensions<> *message) {
  ...
  expect(message->set_extension(protobuf_unittest::repeated_int32_extension{}, std::initializer_list<int32_t>{201, 301})
             .ok());
  expect(message->set_extension(protobuf_unittest::repeated_int64_extension{}, std::initializer_list<int64_t>{202, 302})
             .ok());
  expect(
      message->set_extension(protobuf_unittest::repeated_uint32_extension{}, std::initializer_list<uint32_t>{203, 303})
          .ok());
  expect(
      message->set_extension(protobuf_unittest::repeated_uint64_extension{}, std::initializer_list<uint64_t>{204, 304})
          .ok());
  expect(
      message->set_extension(protobuf_unittest::repeated_sint32_extension{}, std::initializer_list<int32_t>{205, 305})
          .ok());
  expect(
      message->set_extension(protobuf_unittest::repeated_sint64_extension{}, std::initializer_list<int64_t>{206, 306})
          .ok());
  expect(
      message->set_extension(protobuf_unittest::repeated_fixed32_extension{}, std::initializer_list<uint32_t>{207, 307})
          .ok());
  expect(
      message->set_extension(protobuf_unittest::repeated_fixed64_extension{}, std::initializer_list<uint64_t>{208, 308})
          .ok());
  expect(
      message->set_extension(protobuf_unittest::repeated_sfixed32_extension{}, std::initializer_list<int32_t>{209, 309})
          .ok());
  expect(
      message->set_extension(protobuf_unittest::repeated_sfixed64_extension{}, std::initializer_list<int64_t>{210, 310})
          .ok());
  expect(message->set_extension(protobuf_unittest::repeated_float_extension{}, std::initializer_list<float>{211, 311})
             .ok());
  ...
  expect(
      message
          ->set_extension(protobuf_unittest::repeatedgroup_extension{},
                          std::initializer_list<typename repeatedgroup_extension::value_type>{{.a = 217}, {.a = 317}})
          .ok());

  using repeated_nested_message_extension = protobuf_unittest::repeated_nested_message_extension<>;
  expect(message
             ->set_extension(repeated_nested_message_extension{},
                             std::initializer_list<typename repeated_nested_message_extension::value_type>{{.bb = 218},
                                                                                                           {.bb = 318}})
             .ok());
  using repeated_foreign_message_extension = protobuf_unittest::repeated_foreign_message_extension<>;
  expect(message
             ->set_extension(repeated_foreign_message_extension{},
                             std::initializer_list<repeated_foreign_message_extension::value_type>{{.c = 219, .d = {}},
                                                                                                   {.c = 319, .d = {}}})
             .ok());
  using repeated_import_message_extension = protobuf_unittest::repeated_import_message_extension<>;
  expect(message->set_extension(repeated_import_message_extension{}, std::initializer_list<repeated_import_message_extension::value_type>{{.d = 220}, {.d = 320}}).ok());
  expect(message->set_extension(protobuf_unittest::repeated_lazy_message_extension{}, {{.bb = 227}, {.bb = 327}}).ok());

  expect(message
             ->set_extension(protobuf_unittest::repeated_nested_enum_extension{},
...
inline void ExpectAllSet(const protobuf_unittest::TestAllExtensions<> &message) {
  ...
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_int32_extension{}).value(),
                            std::initializer_list<int32_t>{201, 301}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_int64_extension{}).value(),
                            std::initializer_list<int64_t>{202, 302}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_uint32_extension{}).value(),
                            std::initializer_list<uint32_t>{203, 303}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_uint64_extension{}).value(),
                            std::initializer_list<uint64_t>{204, 304}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_sint32_extension{}).value(),
                            std::initializer_list<int32_t>{205, 305}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_sint64_extension{}).value(),
                            std::initializer_list<int64_t>{206, 306}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_fixed32_extension{}).value(),
                            std::initializer_list<uint32_t>{207, 307}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_fixed64_extension{}).value(),
                            std::initializer_list<uint64_t>{208, 308}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_sfixed32_extension{}).value(),
                            std::initializer_list<int32_t>{209, 309}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_sfixed64_extension{}).value(),
                            std::initializer_list<int64_t>{210, 310}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_float_extension{}).value(),
                            std::initializer_list<float>{211, 311}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_double_extension{}).value(),
                            std::initializer_list<double>{212, 312}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_bool_extension{}).value(),
                            std::initializer_list<hpp::proto::boolean>{true, false}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_string_extension{}).value(),
                            std::initializer_list<std::string>{"215", "315"}));
  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_bytes_extension{}).value(),
                            std::initializer_list{"216"_bytes, "316"_bytes}));

  expect(
      std::ranges::equal(message.get_extension(protobuf_unittest::repeatedgroup_extension{}).value(),
                         std::initializer_list<protobuf_unittest::RepeatedGroup_extension<>>{{.a = 217}, {.a = 317}}));

  expect(std::ranges::equal(
      message.get_extension(protobuf_unittest::repeated_nested_message_extension{}).value(),
      std::initializer_list<protobuf_unittest::TestAllTypes<>::NestedMessage>{{.bb = 218}, {.bb = 318}}));

  expect(std::ranges::equal(
      message.get_extension(protobuf_unittest::repeated_foreign_message_extension{}).value(),
      std::initializer_list<protobuf_unittest::ForeignMessage<>>{{.c = 219, .d = {}}, {.c = 319, .d = {}}}));

  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_import_message_extension{}).value(),
                            std::initializer_list<protobuf_unittest_import::ImportMessage<>>{{.d = 220}, {.d = 320}}));

  expect(std::ranges::equal(
      message.get_extension(protobuf_unittest::repeated_lazy_message_extension{}).value(),
      std::initializer_list<protobuf_unittest::TestAllTypes<>::NestedMessage>{{.bb = 227}, {.bb = 327}}));

  expect(std::ranges::equal(
      message.get_extension(protobuf_unittest::repeated_nested_enum_extension{}).value(),
      std::initializer_list<protobuf_unittest::TestAllTypes<>::NestedEnum>{
          protobuf_unittest::TestAllTypes<>::NestedEnum::BAR, protobuf_unittest::TestAllTypes<>::NestedEnum::BAZ}));

  expect(std::ranges::equal(
      message.get_extension(protobuf_unittest::repeated_foreign_enum_extension{}).value(),
      std::initializer_list<protobuf_unittest::ForeignEnum>{protobuf_unittest::ForeignEnum::FOREIGN_BAR,
                                                            protobuf_unittest::ForeignEnum::FOREIGN_BAZ}));

  expect(std::ranges::equal(
      message.get_extension(protobuf_unittest::repeated_import_enum_extension{}).value(),
      std::initializer_list<protobuf_unittest_import::ImportEnum>{protobuf_unittest_import::ImportEnum::IMPORT_BAR,
                                                                  protobuf_unittest_import::ImportEnum::IMPORT_BAZ}));

  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_string_piece_extension{}).value(),
                            std::initializer_list<std::string>{"224", "324"}));

  expect(std::ranges::equal(message.get_extension(protobuf_unittest::repeated_cord_extension{}).value(),
                            std::initializer_list<std::string>{"225", "325"}));
...
inline void SetOneof1(protobuf_unittest::TestOneof2<> *message) {
  using TestOneof2 = protobuf_unittest::TestOneof2<>;
