...
void ExpectMapFieldsSet(const protobuf_unittest::TestMap &message) {
  ...
  expect(eq(2, message.map_int32_int32.size()) >> fatal);
  expect(eq(2, message.map_int64_int64.size()) >> fatal);
  expect(eq(2, message.map_uint32_uint32.size()) >> fatal);
  expect(eq(2, message.map_uint64_uint64.size()) >> fatal);
  expect(eq(2, message.map_sint32_sint32.size()) >> fatal);
  expect(eq(2, message.map_sint64_sint64.size()) >> fatal);
  expect(eq(2, message.map_fixed32_fixed32.size()) >> fatal);
  expect(eq(2, message.map_fixed64_fixed64.size()) >> fatal);
  expect(eq(2, message.map_sfixed32_sfixed32.size()) >> fatal);
  expect(eq(2, message.map_sfixed64_sfixed64.size()) >> fatal);
  expect(eq(2, message.map_int32_float.size()) >> fatal);
  expect(eq(2, message.map_int32_double.size()) >> fatal);
  expect(eq(2, message.map_bool_bool.size()) >> fatal);
  expect(eq(2, message.map_string_string.size()) >> fatal);
  expect(eq(2, message.map_int32_bytes.size()) >> fatal);
  expect(eq(2, message.map_int32_enum.size()) >> fatal);
  expect(eq(2, message.map_int32_foreign_message.size()) >> fatal);
